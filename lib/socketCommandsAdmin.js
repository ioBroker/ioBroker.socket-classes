    ...
    _getAllObjects(socket, callback) {
        ...
        if (this._checkPermissions(socket, 'getObjects', callback)) {
            if (this.objects) {
                ...
                try {
                    this.adapter.getObjectList({include_docs: true}, (error, res) => {
    ...
    _initCommandsAdmin() {
        ...
        this.commands['decrypt'] = (socket, encryptedText, callback) => {
            if (this.secret) {
                ...
                try {
                    this.adapter.getForeignObject('system.config', (error, obj) => {
        ...
        this.commands['getCompactSystemRepositories'] = (socket, callback) => {
            if (this._checkPermissions(socket, 'getObject', callback)) {
                this.adapter.getForeignObject('system.repositories', (error, obj) => {
                    obj && obj.native && obj.native.repositories && Object.keys(obj.native.repositories).forEach(name => {
                        if (obj.native.repositories[name].json) {
                            obj.native.repositories[name].json = null;
                        }
                    });
                    callback(error, obj);
                });
        ...
        this.commands['getCompactHosts'] = (socket, callback) => {
            if (this._checkPermissions(socket, 'getObject', callback)) {
                this.adapter.getObjectView('system', 'host',
                    {startkey: 'system.host.', endkey: 'system.host.\u9999'}, (error, doc) => {
                        if (error) {
                            callback(error);
                        } else {
                            const result = [];
    ...
    __initCommandsStates() {
        ...
        this.commands['getForeignStates'] = (socket, pattern, callback) => {
            if (this._checkPermissions(socket, 'getStates', callback)) {
                if (typeof callback === 'function') {
                    try {
                        this.adapter.getForeignStates(pattern, (error, ...args) =>
                            SocketCommands._fixCallback(callback, error, ...args));
                    } catch (error) {
