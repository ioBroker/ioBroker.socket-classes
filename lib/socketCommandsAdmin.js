...
const socketCommands_1 = __importDefault(require("./socketCommands"));
const axios_1 = __importDefault(require("axios"));
const node_path_1 = require("node:path");
const node_fs_1 = require("node:fs");
const ACL_READ = 4;
const ACL_WRITE = 2;
class SocketCommandsAdmin extends socketCommands_1.default {
    ...
    async updateRatings(uuid) {
        ...
        if (!uuid) {
            ...
            _uuid = ((_a = obj === null || obj === void 0 ? void 0 : obj.native) === null || _a === void 0 ? void 0 : _a.uuid) || '';
        }
        else {
            _uuid = uuid;
        }
        try {
            ...
            if (this.adapter.name === 'admin') {
                ...
                this.context.ratingTimeout = setTimeout(() => {
                    ...
                    this.updateRatings(uuid).then(() => this.adapter.log.info('Adapter rating updated'));
                }, 24 * 3600000);
            }
    ...
    async _readInstanceConfig(id, user, isTab, configs) {
        var _a, _b;
        let obj;
        try {
        ...
        if (obj === null || obj === void 0 ? void 0 : obj.common) {
            ...
            if (((_a = obj.common.adminUI) === null || _a === void 0 ? void 0 : _a.config) === 'materialize') {
                config.materialize = true;
                // @ts-expect-error Fixed in js-controller 7
            }
            else if (((_b = obj.common.adminUI) === null || _b === void 0 ? void 0 : _b.config) === 'json') {
    ...
    _readLicenses(login, password) {
        ...
        return axios_1.default
            ...
            .catch(error => {
            if (error.response) {
                throw new Error((error.response.data && error.response.data.error) ||
                    error.response.data ||
    ...
    // remove this function when js.controller 4.x will be mainstream
    async _updateLicenses(login, password, options) {
        ...
        if (((_a = systemLicenses === null || systemLicenses === void 0 ? void 0 : systemLicenses.native) === null || _a === void 0 ? void 0 : _a.password) && systemLicenses.native.login) {
            // get the secret to decode the password
            if (!this.secret) {
                const systemConfig = await this.adapter.getForeignObjectAsync('system.config', options);
                if ((_b = systemConfig === null || systemConfig === void 0 ? void 0 : systemConfig.native) === null || _b === void 0 ? void 0 : _b.secret) {
            ...
            try {
                const licenses = await this._readLicenses(systemLicenses.native.login, password);
                ...
                if (error.message.includes('Authentication required') ||
                    ...
                    if ((_d = (_c = systemLicenses === null || systemLicenses === void 0 ? void 0 : systemLicenses.native) === null || _c === void 0 ? void 0 : _c.licenses) === null || _d === void 0 ? void 0 : _d.length) {
            ...
            if ((_f = (_e = systemLicenses === null || systemLicenses === void 0 ? void 0 : systemLicenses.native) === null || _e === void 0 ? void 0 : _e.licenses) === null || _f === void 0 ? void 0 : _f.length) {
                systemLicenses.native.licenses = [];
                systemLicenses.native.readTime = new Date().toISOString();
    ...
    _addUser(user, password, options, callback) {
        if (typeof options === 'function') {
            callback = options;
            options = null;
        }
        if (!user.match(/^[-.A-Za-züäößÖÄÜа-яА-Я@+$§0-9=?!&# ]+$/)) {
            return socketCommands_1.default._fixCallback(callback, 'Invalid characters in the name. Only following special characters are allowed: -@+$§=?!&# and letters');
        }
        try {
            this.adapter.getForeignObject(`system.user.${user}`, options, (error, obj) => {
                if (obj) {
                    socketCommands_1.default._fixCallback(callback, 'User yet exists');
                }
                else {
                    try {
                        this.adapter.setForeignObject(`system.user.${user}`, {
                            ...
                            try {
                                ...
                                socketCommands_1.default._fixCallback(callback, error);
                            }
                        });
                    }
                    catch (error) {
    ...
    _delUser(user, options, callback) {
        try {
            this.adapter.getForeignObject(`system.user.${user}`, options, (error, obj) => {
                if (error || !obj) {
                    socketCommands_1.default._fixCallback(callback, 'User does not exist');
                }
                else {
                    if (obj.common.dontDelete) {
                        socketCommands_1.default._fixCallback(callback, 'Cannot delete user, while is system user');
                    }
                    else {
                        try {
    ...
    _addGroup(group, desc, acl, options, callback) {
        ...
        if (!group.match(/^[-.A-Za-züäößÖÄÜа-яА-Я@+$§0-9=?!&#_ ]+$/)) {
            return socketCommands_1.default._fixCallback(callback, 'Invalid characters in the group name. Only following special characters are allowed: -@+$§=?!&# and letters');
        }
        try {
            this.adapter.getForeignObject(`system.group.${group}`, options, (_error, obj) => {
                if (obj) {
                    socketCommands_1.default._fixCallback(callback, 'Group yet exists');
                }
                else {
                    obj = {
                    ...
                    try {
                        this.adapter.setForeignObject(`system.group.${group}`, obj, options, error => socketCommands_1.default._fixCallback(callback, error, obj));
                    }
                    catch (error) {
    ...
    _delGroup(group, options, callback) {
        try {
            this.adapter.getForeignObject(`system.group.${group}`, options, (error, obj) => {
                if (error || !obj) {
                    socketCommands_1.default._fixCallback(callback, 'Group does not exist');
                }
                else {
                    if (obj.common.dontDelete) {
                        socketCommands_1.default._fixCallback(callback, 'Cannot delete group, while is system group');
                    }
                    else {
                        try {
    ...
    _initCommandsUser() {
        this.commands['addUser'] = (socket, user, pass, callback) => {
            ...
            if (this._checkPermissions(socket, 'addUser', callback, user)) {
                this._addUser(user, pass, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
            }
        };
        this.commands['delUser'] = (socket, user, callback) => {
            ...
            if (this._checkPermissions(socket, 'delUser', callback, user)) {
                this._delUser(user, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
            }
        };
        this.commands['addGroup'] = (socket, group, desc, acl, callback) => {
            ...
            if (this._checkPermissions(socket, 'addGroup', callback, group)) {
                this._addGroup(group, desc, acl, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
            }
        };
        this.commands['delGroup'] = (socket, group, callback) => {
            ...
            if (this._checkPermissions(socket, 'delGroup', callback, group)) {
                this._delGroup(group, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
            }
        };
        this.commands['changePassword'] = (socket, user, pass, callback) => {
            ...
            if (user === socket._acl.user || this._checkPermissions(socket, 'changePassword', callback, user)) {
                try {
                    void this.adapter.setPassword(user, pass, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
                }
                catch (error) {
    ...
    _initCommandsAdmin() {
                    ...
                    this.adapter.getObjectView('system', 'host', {}, { user: socket._acl.user }, (error, data) => {
                        ...
                        if (data && data.rows && data.rows.length) {
                            for (let i = 0; i < data.rows.length; i++) {
                                ...
                                if ((_a = obj.native.hardware) === null || _a === void 0 ? void 0 : _a.networkInterfaces) {
                ...
                let timeout = setTimeout(() => {
                    ...
                    if (timeout) {
                        ...
                        try {
                            ...
                            // detect file log
                            if ((_a = config === null || config === void 0 ? void 0 : config.log) === null || _a === void 0 ? void 0 : _a.transport) {
                                            ...
                                            for (let f = 0; f < files.length; f++) {
                                                try {
                                                    if (!files[f].endsWith('-audit.json')) {
                                                        ...
                                                        if (!stat.isDirectory()) {
                                                            (_b = result.list) === null || _b === void 0 ? void 0 : _b.push({
                        ...
                        socketCommands_1.default._fixCallback(callback, result.error, result.list);
                    }
                }, 500);
        ...
        this.commands['delState'] = (socket, id, callback) => {
            ...
            if (this._checkPermissions(socket, 'delState', callback, id)) {
                ...
                try {
                    this.adapter.delForeignState(id, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
                }
                catch (error) {
        ...
        this.commands['cmdExec'] = (socket, host, id, cmd, callback) => {
            ...
            if (this._checkPermissions(socket, 'cmdExec', callback, cmd)) {
                ...
                try {
                    ...
                    socketCommands_1.default._fixCallback(callback, null);
                }
                catch (error) {
        ...
        this.commands['getRatings'] = (_socket, update, callback) => {
            ...
            if (update || !this.context.ratings) {
                this.updateRatings().then(() => socketCommands_1.default._fixCallback(callback, null, this.context.ratings));
            }
            else {
                socketCommands_1.default._fixCallback(callback, null, this.context.ratings);
            }
        };
        this.commands['getCurrentInstance'] = (_socket, callback) => {
            ...
            socketCommands_1.default._fixCallback(callback, null, this.adapter.namespace);
        };
        this.commands['decrypt'] = (socket, encryptedText, callback) => {
            ...
            if (this.secret) {
                socketCommands_1.default._fixCallback(callback, null, this.adapter.decrypt(this.secret, encryptedText));
            }
            else {
                try {
        ...
        this.commands['encrypt'] = (socket, plainText, callback) => {
            ...
            if (this.secret) {
                socketCommands_1.default._fixCallback(callback, null, this.adapter.encrypt(this.secret, plainText));
            }
            else {
                this.adapter.getForeignObject('system.config', { user: socket._acl.user }, (error, obj) => {
                    if (obj && obj.native && obj.native.secret) {
                        ...
                        try {
                            ...
                            socketCommands_1.default._fixCallback(callback, null, encrypted);
                        }
                        catch (error) {
        ...
        this.commands['getIsEasyModeStrict'] = (_socket, callback) => {
            ...
            socketCommands_1.default._fixCallback(callback, null, this.adapter.config.accessLimit);
        };
        this.commands['getEasyMode'] = (socket, callback) => {
            ...
            if (this._checkPermissions(socket, 'getObject', callback)) {
                ...
                if (this.adapter.config.accessLimit) {
                    const configs = [];
                    const promises = [];
                    (_a = this.adapter.config.accessAllowedConfigs) === null || _a === void 0 ? void 0 : _a.forEach(id => promises.push(this._readInstanceConfig(id, user, false, configs)));
                    (_b = this.adapter.config.accessAllowedTabs) === null || _b === void 0 ? void 0 : _b.forEach(id => promises.push(this._readInstanceConfig(id, user, true, configs)));
                    ...
                    this.adapter.getObjectView('system', 'instance', { startkey: 'system.adapter.', endkey: 'system.adapter.\u9999' }, { user }, (error, doc) => {
                        ...
                        const configs = [];
                        const promises = [];
                        if (!error && ((_a = doc === null || doc === void 0 ? void 0 : doc.rows) === null || _a === void 0 ? void 0 : _a.length)) {
                        ...
                        Promise.all(promises).then(() => socketCommands_1.default._fixCallback(callback, null, {
                            strict: false,
                            configs,
                        }));
                    });
            ...
            if (typeof callback === 'function' && this._checkPermissions(socket, 'getObject', callback)) {
                this.adapter.getObjectView('system', 'adapter', {
                    ...
                    if (error) {
                        ...
                        callback(null, doc === null || doc === void 0 ? void 0 : doc.rows.filter(obj => { var _a; return obj && (!adapterName || ((_a = obj.value.common) === null || _a === void 0 ? void 0 : _a.name) === this.adapterName); }).map(item => {
            ...
            if (this._checkPermissions(socket, 'setObject', callback, login, password)) {
                if (this.adapter.supportsFeature('CONTROLLER_LICENSE_MANAGER')) {
                    let timeout = setTimeout(() => {
                        if (timeout) {
                            timeout = null;
                            socketCommands_1.default._fixCallback(callback, 'updateLicenses timeout');
                        }
                    }, 7000);
                    ...
                    this._sendToHost(this.adapter.common.host, 'updateLicenses', { login, password }, result => {
                        if (timeout) {
                            clearTimeout(timeout);
                            timeout = null;
                            socketCommands_1.default._fixCallback(callback, result.error, result && result.result);
                        }
                    });
                }
                else {
                    ...
                    this._updateLicenses(login, password, { user: socket._acl.user })
                        .then(licenses => socketCommands_1.default._fixCallback(callback, null, licenses))
                        .catch(error => socketCommands_1.default._fixCallback(callback, error));
                }
            }
        };
                ...
                if (this._checkPermissions(socket, 'getObject', callback)) {
                    this.adapter.getObjectView('system', 'instance', { startkey: `system.adapter.`, endkey: `system.adapter.\u9999` }, { user: socket._acl.user }, (error, doc) => {
                        if (error) {
                            ...
                            socketCommands_1.default._fixCallback(callback, null, result);
                        }
                    });
                }
            }
        ...
        this.commands['getCompactSystemConfig'] = (socket, callback) => {
            ...
            if (this._checkPermissions(socket, 'getObject', callback)) {
                this.adapter.getForeignObject('system.config', { user: socket._acl.user }, (error, obj) => {
                    ...
                    socketCommands_1.default._fixCallback(callback, error, obj);
                });
            }
        };
                        ...
                        doc === null || doc === void 0 ? void 0 : doc.rows.map(item => {
                            ...
                            if (host) {
                                ...
                                result.push({
                                    ...
                                    native: {
                                        hardware: {
                                            networkInterfaces: ((_b = (_a = host.native) === null || _a === void 0 ? void 0 : _a.hardware) === null || _b === void 0 ? void 0 : _b.networkInterfaces) || undefined,
                                        },
                                    },
                                });
                            }
        ...
        this.commands['writeFile'] = (socket, _adapter, fileName, data64, options, callback) => {
            ...
            if (this._checkPermissions(socket, 'writeFile', callback, fileName)) {
                ...
                try {
                    ...
                    this.adapter.writeFile(_adapter, fileName, buffer, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
                }
                catch (error) {
    ...
    __initCommandsObjects() {
        ...
        this.commands['getObjects'] = (socket, list, callback) => {
            ...
            if (list === null || list === void 0 ? void 0 : list.length) {
                if (this._checkPermissions(socket, 'getObject', callback)) {
                    if (typeof callback === 'function') {
                        try {
                            this.adapter.getForeignObjects(list, { user: socket._acl.user }, (error, objs) => socketCommands_1.default._fixCallback(callback, error, objs));
                        }
                        catch (error) {
        ...
        this.commands['extendObject'] = (socket, id, obj, callback) => {
            ...
            if (this._checkPermissions(socket, 'extendObject', callback, id)) {
                try {
                    this.adapter.extendForeignObject(id, obj, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
                }
                catch (error) {
        ...
        this.commands['getForeignObjects'] = (socket, pattern, type, callback) => {
            ...
            if (this._checkPermissions(socket, 'getObjects', callback)) {
                ...
                if (typeof callback === 'function') {
                    try {
                        this.adapter.getForeignObjects(pattern, type, { user: socket._acl.user }, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
                    }
                    catch (error) {
                        this.adapter.log.error(`[extendObject] ERROR: ${error}`);
                        socketCommands_1.default._fixCallback(callback, error);
        ...
        this.commands['delObject'] = (socket, id, options, callback) => {
            ...
            if (this._checkPermissions(socket, 'delObject', callback, id)) {
                ...
                try {
                    ...
                    this.adapter.delForeignObject(id, options, (error, ...args) => socketCommands_1.default._fixCallback(callback, error, ...args));
                }
                catch (error) {
...
exports.default = SocketCommandsAdmin;
