...
class SocketCommon {
    ...
    start(server, socketClass, authOptions, socketOptions) {
        ...
        this.commands = this.commands || new SocketCommands(this.adapter, socket => this.__updateSession(socket), this.context);
        ...
        if (socketClass) {
            if (!server.__inited) {
                if (typeof socketClass.constructor === 'function') {
                    // New ws
                    this.server = new socketClass(server, socketOptions);
                } else if (typeof socketClass.listen === 'function') {
                    // old socket.io@2.x and ws
                    this.server = socketClass.listen(server, socketOptions);
                } else if (socketClass.constructor) {
                    // iobroker socket
                    this.server = new socketClass(server, socketOptions);
                } else {
