...
class SocketCommands {
    ...
    subscribe(socket, type, pattern, patternFile) {
        ...
        if (this.subscribes[type][key] === undefined) {
            ...
            if (type === 'stateChange') {
                this.adapter.subscribeForeignStates(pattern, {user: socket._acl.user});
            } else if (type === 'objectChange') {
                this.adapter.subscribeForeignObjects && this.adapter.subscribeForeignObjects(pattern, {user: socket._acl.user});
            } else if (type === 'log') {
                if (!this.logEnabled && this.adapter.requireLog) {
                    this.logEnabled = true;
                    this.adapter.requireLog(true); // , {user: socket._acl.user}
                }
            } else if (type === 'fileChange') {
                this.adapter.subscribeForeignFiles && this.adapter.subscribeForeignFiles(pattern, patternFile, {user: socket._acl.user});
            }
        } else {
        ...
        if (socket && typeof socket === 'object') {
                    ...
                    if (this.subscribes[type][key] !== undefined) {
                        ...
                        if (this.subscribes[type][key] <= 0) {
                            if (type === 'stateChange') {
                                ...
                                this.adapter.unsubscribeForeignStates(pattern, {user: socket._acl.user});
                            } else if (type === 'objectChange') {
                                //console.log((socket._name || socket.id) + ' unsubscribeForeignObjects ' + pattern);
                                this.adapter.unsubscribeForeignObjects && this.adapter.unsubscribeForeignObjects(pattern, {user: socket._acl.user});
                            } else if (type === 'log') {
                                //console.log((socket._name || socket.id) + ' requireLog false');
                                if (this.logEnabled && this.adapter.requireLog) {
                                    this.logEnabled = false;
                                    this.adapter.requireLog(false); // , {user: socket._acl.user}
                                }
                            } else if (type === 'fileChange') {
                                //console.log((socket._name || socket.id) + ' requireLog false');
                                this.adapter.unsubscribeForeignFiles && this.adapter.unsubscribeForeignFiles(pattern, patternFile, {user: socket._acl.user});
                            }
            ...
            Object.keys(this.subscribes[type]).forEach(pattern => {
                if (type === 'stateChange') {
                    ...
                    this.adapter.unsubscribeForeignFiles && this.adapter.unsubscribeForeignFiles(id, fileName, {user: socket._acl.user});
                }
            });
        ...
        for (let i = 0; i < socket.subscribe[type].length; i++) {
            ...
            if (this.subscribes[type][pattern] === undefined) {
                ...
                if (type === 'stateChange') {
                    ...
                    const [id, fileName] = pattern.split('####');
                    this.adapter.subscribeForeignFiles && this.adapter.subscribeForeignFiles(id, fileName, {user: socket._acl.user});
                }
            } else {
