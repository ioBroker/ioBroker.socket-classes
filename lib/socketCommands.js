    ...
    async subscribe(socket, type, pattern, patternFile) {
        ...
        if (this.subscribes[type][key] === undefined) {
            ...
            if (type === 'stateChange') {
                ...
                try {
                    await this.adapter.subscribeForeignObjectsAsync(pattern, options);
                } catch (e) {
                    this.adapter.log.error(`Cannot subscribe "${pattern}": ${e.message}`)
                }
            } else if (type === 'log') {
        ...
        if (socket && typeof socket === 'object') {
                    ...
                    if (this.subscribes[type][key] !== undefined) {
                        ...
                        if (this.subscribes[type][key] <= 0) {
                            if (type === 'stateChange') {
                                ...
                                try {
                                   await this.adapter.unsubscribeForeignObjectsAsync(pattern, options);
                                } catch (e) {
                                    this.adapter.log.error(`Cannot unsubscribe "${pattern}": ${e.message}`)
                                }
                            } else if (type === 'log') {
                                ...
                                try {
                                    await this.adapter.unsubscribeForeignFiles(pattern, patternFile, options);
                                } catch (e) {
                                    this.adapter.log.error(`Cannot unsubscribe "${pattern}": ${e.message}`)
                                }
                            }
           ...
           for (const pattern of Object.keys(this.subscribes[type])) {
                if (type === 'stateChange') {
                    ...
                    try {
                        await this.adapter.unsubscribeForeignFiles(id, fileName, options);
                    } catch (e) {
                        this.adapter.log.error(`Cannot unsubscribe "${pattern}": ${e.message}`)
                    }
                }
        ...
        for (let i = 0; i < socket.subscribe[type].length; i++) {
            ...
            if (this.subscribes[type][pattern] === undefined) {
                ...
                if (type === 'stateChange') {
                    ...
                    const [id, fileName] = pattern.split('####');
                    try {
                        await this.adapter.subscribeForeignFiles(id, fileName, options);
                    } catch (e) {
                        this.adapter.log.error(`Cannot subscribe "${pattern}": ${e.message}`)
                    }
                }
