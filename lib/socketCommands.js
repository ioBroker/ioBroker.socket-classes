...
class SocketCommands {
    ...
    publishInstanceMessage(socket, sourceInstance, messageType, data) {
        ...
        this.adapter.sendTo(sourceInstance, 'clientSubscribeError', {type: messageType, sid: socket.id, reason: 'no one subscribed'});
        return false;
    }
    ...
    async subscribe(socket, type, pattern, patternFile) {
        if (!pattern) {
        ...
        if (this.subscribes[type][key] === undefined) {
            ...
            if (type === 'stateChange') {
                try {
                    await this.adapter.subscribeForeignStatesAsync(pattern, options);
                } catch (e) {
                    this.adapter.log.error(`Invalid pattern on subscribe: ${e.message}`)
                    return
                }
                ...
                try {
                    this.adapter.subscribeForeignObjects && await this.adapter.subscribeForeignObjects(pattern, options);
                } catch (e) {
                    this.adapter.log.error(`Invalid pattern on subscribe: ${e.message}`)
                    return
                }
                ...
                try {
                   await this.adapter.subscribeForeignFiles(pattern, patternFile, options);
                } catch (e) {
                    this.adapter.log.error(`Invalid pattern on subscribe: ${e.message}`)
                    return
                }
        ...
        if (socket && typeof socket === 'object') {
                ...
                if (socket.subscribe[type][i].pattern === key) {
                    ...
                    if (this.subscribes[type][key] !== undefined) {
                        ...
                        if (this.subscribes[type][key] <= 0) {
                            if (type === 'stateChange') {
                                try {
                                    await this.adapter.unsubscribeForeignStatesAsync(pattern, options);
                                } catch (e) {
                                    this.adapter.log.error(`Invalid pattern on unsubscribe: ${e.message}`)
                                    return
                                }
                            } else if (type === 'objectChange') {
                                try {
                                    this.adapter.unsubscribeForeignObjects && await this.adapter.unsubscribeForeignObjectsAsync(pattern, options);
                                } catch (e) {
                                    this.adapter.log.error(`Invalid pattern on unsubscribe: ${e.message}`)
                                    return
                                }
                                ...
                                try {
                                    this.adapter.unsubscribeForeignFiles && await this.adapter.unsubscribeForeignFiles(pattern, patternFile, options);
                                } catch (e) {
                                    this.adapter.log.error(`Invalid pattern on unsubscribe: ${e.message}`)
                                    return
                                }
           ...
           for (const pattern of Object.keys(this.subscribes[type])) {
                if (type === 'stateChange') {
                    ...
                    try {
                        this.adapter.unsubscribeForeignFiles && await this.adapter.unsubscribeForeignFiles(id, fileName, options);
                    } catch (e) {
                        this.adapter.log.error(`Invalid pattern on unsubscribe: ${e.message}`)
                        return
                    }
        ...
        for (let i = 0; i < socket.subscribe[type].length; i++) {
            ...
            if (this.subscribes[type][pattern] === undefined) {
                ...
                if (type === 'stateChange') {
                    ...
                    try {
                        this.adapter.subscribeForeignObjects && await this.adapter.subscribeForeignObjectsAsync(pattern, options);
                    } catch (e) {
                        this.adapter.log.error(`Invalid pattern on subscribe: ${e.message}`)
                        return
                    }
                    ...
                    const [id, fileName] = pattern.split('####');
                    try {
                        this.adapter.subscribeForeignFiles && await this.adapter.subscribeForeignFiles(id, fileName, options);
                    } catch (e) {
                        this.adapter.log.error(`Invalid pattern on subscribe: ${e.message}`)
                        return
                    }
        ...
        this.commands['clientUnsubscribe'] = (socket, targetInstance, messageType, callback) => {
            ...
            if (this.clientSubscribes[sid] && this.clientSubscribes[sid][targetInstance]) {
                ...
                if (pos !== -1) {
                    ...
                    this.adapter.sendTo(targetInstance, 'clientUnsubscribe', {type: [messageType], sid, reason: 'client'});
                    SocketCommands._fixCallback(callback, null, true);
                    return;
                }
            }
        ...
        if (this.clientSubscribes[socketId]) {
            Object.keys(this.clientSubscribes[socketId]).forEach(targetInstance => {
                this.adapter.sendTo(targetInstance, 'clientUnsubscribe', {
                    message: this.clientSubscribes[socketId][targetInstance],
                    sid: socketId,
                    reason: 'disconnect',
                });
            });
